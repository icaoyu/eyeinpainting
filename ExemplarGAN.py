...
from utils1 import save_images
...
class ExemplarGAN(object):
    ...
    def test(self, test_step,img_path,eximg_path,detector,predictor):
        ...
        with tf.Session(config=config) as sess:
            ...
            for j in range(batch_num):
                test_data_list, batch_eye_pos, test_ex_list, test_eye_pos=self.data_ob.getTestData(img_path,eximg_path,detector, predictor)
                ...
                outname = self.get_filename(test_data_list[0])+"_"+self.get_filename(test_ex_list[0])
                print()
                # save_images(img,[1,1],'{}/masked/{:02d}.jpg'.format(self.sample_path, j))
                # save_images(img, [1, 1], '{}/ex/{:02d}.jpg'.format(self.sample_path, j))
                # save_images(img, [1, 1], '{}/output/out_{:02d}.jpg'.format(self.sample_path, j))
                save_images(output_concat, [output_concat.shape[0] / 4, 4],
                            '{}/out_{}.jpg'.format(self.sample_path, outname))
    ...
    def testdb(self):
        train_data_list, batch_eye_pos, batch_train_ex_list, batch_ex_eye_pos = self.data_ob.getNextBatch(1,
                                                                                                          self.batch_size)

        # for i in range(10):
        print(train_data_list,batch_train_ex_list,batch_eye_pos,batch_ex_eye_pos)
            # print(train_data_list, batch_eye_pos, batch_train_ex_list, batch_ex_eye_pos)                                                                                           self.batch_size)
        ...
        with tf.Session(config=config) as sess:
            ...
            while step <= self.max_iters:
                ...
                for i in range(self.n_critic):
                    ...
                    # optimize D
                    # print('shape:',batch_images_array.shape,batch_exem_array.shape,batch_eye_pos.shape,batch_ex_eye_pos.shape)

                    # print(self.input_img.shape,self.img_mask)
